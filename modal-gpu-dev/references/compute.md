...
Optimize for cheapest valid result, not fastest epoch:
...
For workloads that need more than 8 GPUs, use multi-node with `@modal.experimental.clustered`. This gang-schedules multiple containers and connects them via RDMA (up to 3,200 Gbps). See the `modal-gpu-experiment` skill for details.
...
_SANDBOX_GPU=A10 python -m modal run modal-gpu-dev/tools/gpu_sandbox.py \
  --key-path ~/.ssh/id_ed25519.pub --gpu A10
```
